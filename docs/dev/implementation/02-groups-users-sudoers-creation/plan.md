...
- [Step 4 - Vault setup script](#step-4---vault-setup-script)
- [Step 5 - Role: groups](#step-5---role-groups)
- [Step 6 - Role: users](#step-6---role-users)
- [Step 7 - Role: sudoers](#step-7---role-sudoers)
- [Step 8 - Playbook and operator entry point](#step-8---playbook-and-operator-entry-point)
- [Step 9 - Smoke test against a real VM](#step-9---smoke-test-against-a-real-vm)
- [Step 10 - README and per-step docs](#step-10---readme-and-per-step-docs)
- [Step 11 - E2E test layer for the Ansible users path](#step-11---e2e-test-layer-for-the-ansible-users-path)
...
commands with no branching logic worth unit-testing in isolation. Step 9
...
Add a "Bridge contract" subsection in step-10 README work documenting the
...
top-level README in step 10 links to them.
...
## Step 8 - Playbook and operator entry point
...
Covered by step 9 (smoke test) — the playbook itself has no logic
beyond role ordering.

---

## Step 9 - Smoke test against a real VM
...
- Smoke test is **manual** for v1. It is not in CI. It runs once on the
  ...
  step-10's README as a recorded transcript. Automating it requires
...
## Step 10 - README and per-step docs
...
- Per-role `README.md`s (already created in steps 5-7, finalised here).
...
## Step 11 - E2E test layer for the Ansible users path
